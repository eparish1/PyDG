...
def inviscidFlux(main,eqns,fluxVar,var,args=None):
  ...
  ny = np.array([0,1,0])
  nz = np.array([0,0,1])
  fluxVar.fRS[:,:,:,:,0:-1,:,:] = eqns.inviscidFlux(main,var.uR[:,:,:,:,0:-1,:,:],var.uL[:,:,:,:,1::,:,:],main.a.pR[:,:,:,0:-1,:,:],main.a.pL[:,:,:,1::,:,:],nx)
  fluxVar.fRS[:,:,:,:,  -1,:,:] = eqns.inviscidFlux(main,var.uR[:,:,:,:,  -1,:,:],var.uR_edge,main.a.pR[:,:,:,-1,:,:],main.a.pR_edge,nx)
  fluxVar.fLS[:,:,:,:,1:: ,:,:] = fluxVar.fRS[:,:,:,:,0:-1,:,:]
  fluxVar.fLS[:,:,:,:,0   ,:,:] = eqns.inviscidFlux(main,var.uL_edge,var.uL[:,:,:,:,0,:,:],main.a.pL_edge,main.a.pL[:,:,:,0,:,:],nx)
...
def inviscidFluxGen(main,eqns,fluxVar,var,args):
  ...
  fluxVar.fRS[:,:,:,:,0:-1,:,:] = eqns.inviscidFlux(main,var.uR[:,:,:,:,0:-1,:,:],var.uL[:,:,:,:,1::,:,:],main.cgas_field_LR,nx,fluxArgs)
  fluxArgs = []
  ...
  fluxVar.fRS[:,:,:,:,  -1,:,:] = eqns.inviscidFlux(main,var.uR[:,:,:,:,  -1,:,:],var.uR_edge,main.cgas_field_R_edge,nx,fluxArgs)
  ...
  fluxVar.fLS[:,:,:,:,0   ,:,:] = eqns.inviscidFlux(main,var.uL_edge,var.uL[:,:,:,:,0,:,:],main.cgas_field_L_edge,nx,fluxArgs)
  ## Get the up and down fluxes
  fluxArgs = []
  for i in range(0,nargs):
    fluxArgs.append(argsU[i][:,:,:,:,:,0:-1,:])
    fluxArgs.append(argsD[i][:,:,:,:,:,1::,:])
  fluxVar.fUS[:,:,:,:,:,0:-1,:] = eqns.inviscidFlux(main,var.uU[:,:,:,:,:,0:-1,:],var.uD[:,:,:,:,:,1::,:],main.cgas_field_UD,ny,fluxArgs)
  fluxArgs = []
  for i in range(0,nargs):
    fluxArgs.append(argsU[i][:,:,:,:,:,  -1,:])
    fluxArgs.append(argsU_edge[i])
  fluxVar.fUS[:,:,:,:,:,  -1,:] = eqns.inviscidFlux(main,var.uU[:,:,:,:,:,  -1,:],var.uU_edge,main.cgas_field_U_edge,ny,fluxArgs)
  fluxVar.fDS[:,:,:,:,:,1:: ,:] = fluxVar.fUS[:,:,:,:,:,0:-1,:] 
  fluxArgs = []
  for i in range(0,nargs):
    fluxArgs.append(argsD_edge[i])
    fluxArgs.append(argsD[i][:,:,:,:,:,0,:])
  fluxVar.fDS[:,:,:,:,:,0   ,:] = eqns.inviscidFlux(main,var.uD_edge,var.uD[:,:,:,:,:,0,:],main.cgas_field_D_edge,ny,fluxArgs)
  ## Get the front and back fluxes
  fluxArgs = []
  for i in range(0,nargs):
    fluxArgs.append(argsF[i][:,:,:,:,:,:,0:-1])
    fluxArgs.append(argsB[i][:,:,:,:,:,:,1::])
  fluxVar.fFS[:,:,:,:,:,:,0:-1] = eqns.inviscidFlux(main,var.uF[:,:,:,:,:,:,0:-1],var.uB[:,:,:,:,:,:,1::],main.cgas_field_FB,nz,fluxArgs)
  fluxArgs = []
  for i in range(0,nargs):
    fluxArgs.append(argsF[i][:,:,:,:,:,:,-1])
    fluxArgs.append(argsF_edge[i])
  fluxVar.fFS[:,:,:,:,:,:,  -1] = eqns.inviscidFlux(main,var.uF[:,:,:,:,:,:,  -1],var.uF_edge,main.cgas_field_F_edge,nz,fluxArgs)
  fluxVar.fBS[:,:,:,:,:,:,1:: ] = fluxVar.fFS[:,:,:,:,:,:,0:-1] 
  fluxArgs = []
  for i in range(0,nargs):
    fluxArgs.append(argsB_edge[i])
    fluxArgs.append(argsB[i][:,:,:,:,:,:,0])
  fluxVar.fBS[:,:,:,:,:,:,0   ] = eqns.inviscidFlux(main,var.uB_edge,var.uB[:,:,:,:,:,:,0],main.cgas_field_B_edge,nz,fluxArgs)
