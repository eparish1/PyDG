...
#      delta = 3
    ...
    sol = linear_solver.solve(MF_Jacobian, -Rstarn.flatten(), old.flatten(),main_coarse,MF_Jacobian_args,PC,PC_args,loc_tol,1,40,0)
...
def newtonSolver_POD(unsteadyResidual,MF_Jacobian,main,linear_solver,sparse_quadrature,eqns,PC=None):
  if (sparse_quadrature):
    coarsen = 2
    quadpoints_coarsen = np.fmax(main.quadpoints/(coarsen),1)
    quadpoints_coarsen[-1] = main.quadpoints[-1]
  ...
  while (Rstar_glob >= 1e-8 and Rstar_glob/Rstar_glob0 > 1e-4):
    ...
    PC_args = [1,loc_tol,PC_iteration]
    RHS = -Rstarn.flatten()
    old = np.zeros(np.size(RHS))
    sol = linear_solver.solve(MF_Jacobian,RHS , old.flatten(),main_coarse,MF_Jacobian_args,PC,PC_args,loc_tol,1,20,0)
    main.a.a[:] = an[:] + 1.0*np.reshape(np.dot(main.V,sol),np.shape(main.a.a))
    an[:] = main.a.a[:]
    Rstarn,Rn,Rstar_glob = unsteadyResidual(main,main.a.a)
    resid_hist = np.append(resid_hist,Rstar_glob)
    t_hist = np.append(t_hist,time.time() - tnls)
