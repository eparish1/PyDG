...
def evalFluxXYZ_shallowWater(eqns,main,u,fx,fy,fz,args):
  #print(eqns.params)
  es = 1.e-30
  g = eqns.params[0] 
  h = u[0]
  ...
  #u = rho_eta/rho eta U
  fx[0] = u[1]
  fx[1] = hU*hU/h + 0.5*g*(h**2)# - h0**2)
  fx[2] = hU*hV/h

  fy[0] = u[2]
  fy[1] = hU*hV/(h)
  fy[2] = hV*hV/(h) + 0.5*g*(h**2)# - h0**2)*g 
  #print(np.linalg.norm(fx),np.linalg.norm(fy))
...
#
  ...
  #F[0]    = 0.5*(FL[0]+FR[0])-0.5*smax*( (UR[0] - hR0) - (UL[0] - hL0) )
  F[0]    = 0.5*(FL[0]+FR[0])-0.5*smax*(UR[0] - UL[0])
  F[1]    = 0.5*(FL[1]+FR[1])-0.5*smax*(UR[1] - UL[1])
  F[2]    = 0.5*(FL[2]+FR[2])-0.5*smax*(UR[2] - UL[2])
  #print('hi',np.linalg.norm(F),np.linalg.norm(FR),np.linalg.norm(FL),np.linalg.norm(FL+FR))
  return F
...
def swe_rusanovFlux_height(eqns,F,main,UL,UR,n,args=None):
# PURPOSE: This function calculates the flux for the SWE equations
# using the rusanov flux function
#
# INPUTS:
