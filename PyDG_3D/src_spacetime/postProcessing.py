      ...
      for i in range(0,end,skip):
        ...
        for j in range(0,n_blocks):
          ...
          if (os.path.isfile(sol_str)):
            ...
            u    = sol['U'][0]
            gridToVTK(string, x[j], y[j], z[j], pointData = {"u" : u})    
    self.writeAllToParaview = writeAllToParaview
      ...
      for i in range(0,end,skip):
        ...
        for j in range(0,n_blocks):
          ...
          if (os.path.isfile(sol_str)):
            ...
            v      = sol['U'][2]/sol['U'][0]
            eta      = sol['U'][0] - sol['U'][0]
            gridToVTK(string, x[j], y[j], z[j], pointData = {"h" : h,  \
                                                               "u" : u,    \
                                                               "v" : v,    \
                                                               "eta": eta} )   
...
class postProcessor:
  def __init__(self):
    def writeAllToParaview(self,start=0,end=1000000,skip=1):
      n_blocks = 0
      check = 0
      x = []
      y = []
      ...
      for i in range(0,end,skip):
        ...
        for j in range(0,n_blocks):
          ...
          sol_str = 'npsol_block' + str(j) + '_'  + str(i) + '.npz'
          PVstring = 'PVsol' + str(j) + '_' + str(i) + '.vts'
         
          if (os.path.isfile(sol_str) and os.path.isfile(PVstring) == False):
            print('found ' + sol_str)
            sol = np.load('npsol_block' + str(j) + '_' + str(i) + '.npz')
            string = 'PVsol' + str(j) + '_' + str(i)
...
if (eqn_type[0] == 'Navier-Stokes'):
  ...
  check = 1
if (eqn_type[0] == 'Shallow-Water'):
  print('Post Processing for Shallow Water Equations')
  postProcess = postProcessor_swe()
  check = 1
if (check == 0):
