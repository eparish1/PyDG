...
def getMassMatrix(main):
  ...
  #tmp = volIntegrateGlob_einsumMM2(main,main.Jdet[None,:,:,:,None,:,:,:,None] )
  z =  volIntegrateGlob_einsumMM2(main,main.Jdet[None,:,:,:,None,:,:,:,None],main.w0,main.w1,main.w2,main.w3) 
  ...
  return np.reshape(tmp,np.shape(M)) , np.reshape(M2,np.shape(M))
...
def computeJacobian(X_el,zeta0,zeta1,zeta2):
  ...
  J = np.zeros((3,3,orderx,ordery,orderz,Nelx,Nely,Nelz),order='C')
  JRL = np.zeros((3,2,ordery,orderz,Nelx+1,Nely,Nelz),order='C')
  JUD = np.zeros((3,2,orderx,orderz,Nelx,Nely+1,Nelz),order='C')
  JFB = np.zeros((3,2,orderx,ordery,Nelx,Nely,Nelz+1),order='C')
  JRLinv = np.zeros((3,2,ordery,orderz,Nelx+1,Nely,Nelz),order='C')
  JUDinv = np.zeros((3,2,orderx,orderz,Nelx,Nely+1,Nelz),order='C')
  JFBinv = np.zeros((3,2,orderx,ordery,Nelx,Nely,Nelz+1),order='C')
  JRLdet = np.zeros((ordery,orderz,Nelx+1,Nely,Nelz),order='C')
  JUDdet = np.zeros((orderx,orderz,Nelx,Nely+1,Nelz),order='C')
  JFBdet = np.zeros((orderx,ordery,Nelx,Nely,Nelz+1),order='C')
  ...
  return J,Jinv,Jdet,J_edge_det,J_edge_inv,normals
