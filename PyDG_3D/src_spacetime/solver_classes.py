...
class timeschemes:
  def __init__(self,main,time_str='ExplicitRK4',lsolver_str='GMRes',nlsolver_str='Newton'):
    ...
    if (time_str == 'SSP_RK3_POD_DEIM'):
      check_t = 0
      self.advanceSol = SSP_RK3_POD_DEIM
      self.args = None
      V = np.load('pod_basis.npz')['V']
      n_basis = np.shape(V)[1]
      V2 = np.zeros((main.nvars,main.order[0],main.order[1],main.order[2],main.order[3],main.Npx,main.Npy,main.Npz,main.Npt,n_basis))
      ...
      main.DP = np.load('mats_deim.npz')['DP']
    ...
    if (time_str == 'SpaceTimeEntropy'):
      check_t = 0
      self.advanceSol = spaceTimeEntropy
      self.linear_solver = linearSolver(lsolver_str)
      self.nonlinear_solver = nonlinearSolver(nlsolver_str)
      self.sparse_quadrature = False
      self.args = [self.nonlinear_solver,self.linear_solver,self.sparse_quadrature]
...
class nonlinearSolver:
  def __init__(self,SolverType='Newton',rtol=1e-8,printnorm=1):
    ...
    if (SolverType == 'Newton_POD'):
      self.solve = newtonSolver_POD
      self.rtol=rtol
      self.printnorm = printnorm
    if (SolverType == 'NEJ'):
